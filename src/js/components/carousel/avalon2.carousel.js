 ...
 *
 */
var tlp=require('./avalon2.carousel.html');
avalon.component('ms-carousel',{
	...
	defaults : {
			...
			if (vm.effect === "slide") {
				...
				var go = function () {
					...
					//队列还未到终点，在移动过程中
						...
						vm.panelOffsetX = vm.Tween(vm.easing, currentTime,
				...
				var go = function () {
					vm.animated = false
					vm.fadein = vm.Tween(vm.easing, currentTime, 0, 1, duringTime) //移动
					vm.fadeout = vm.Tween(vm.easing, currentTime, 1, -1, duringTime) //移动
					if (currentTime < duringTime) {
						currentTime += 1
						requestAnimationFrame(go)
						vm.animated = true
		...
		onInit: function () {
			var vm = this;
			vm.innerWidth=vm.pictureWidth * vm.pictures.length
			// 处理carousel尺寸
		 ...
		 */
		Tween:function(eatingType, curTime, startPos, distance, duration) {
	switch (eatingType) {
		case "linear":
		...
		,autoPlay:function () {
			...
			if (this.timer === null && this.autoSlide) {
				function play() {
