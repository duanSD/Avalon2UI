...
var template=require('./avalon.menu.html');
require('./avalon.menu.css');
require('../chameleon/oniui-common.css');
    var counter = 0;
    function getCnt() {
        return counter++
    }
...
avalon.component('ms-menu',{
    ...
    defaults : {
        ...
        ////配轩参数
        ,_oldActive:false
        ,_subMenus : {} // 维护一个子menu列表，用对象，更好读写
        ,$skipArray: ["widgetElement", "template", "_subMenus", "_oldActive", "rootElement"]
        ,onInit: function () {
            var vmodel=this;
            vmodel.event = vmodel.event === "mouseover" ? "mouseenter" : vmodel.event
            //方便用户对原始模板进行修改,提高定制性
            vmodel.template = vmodel.template.replace(/\{\{MS_OPTION_EVENT\}\}/, options.event).replace(/\{\{\MS_OPTION_CNT}\}/g, counter)
            if(vmodel.data == void 0) {
                vmodel.data = buildData(this.$element.children, vmodel)
            } else {
                formateData(vmodel.data)
            }
            vmodel._oldActive = this.active
            if(inited) return
            inited = true;
            vmodel.widgetElement = this.$element
            vmodel.rootElement = this.$element
        ...
        ,_canActive : function(menu, index) {
        ...
        // 冒泡到第一级menu进行处理
        ,_onSelect: function (e, activeData) {
            if(vmodel._depth === 1) {
        ...
        ,_getNodeByData: function (activeData) {
        ...
        _restMenu: function(model) {
            model.menuResetter(model)
            model._oldActive = model.active
        },
        _cutCounter : avalon.noop,
        _canRemove : avalon.noop
    }
});
