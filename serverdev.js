...
// 配置代码自动编译和热替换插件
/*config.entry.unshift('webpack-dev-server/client?http://localhost:9090', "webpack/hot/only-dev-server"); 要手动刷新页面
config.plugins.push(new webpack.HotModuleReplacementPlugin());*/
for (var i in config.entry) {
    config.entry[i].unshift('webpack-dev-server/client?http://localhost:' + devPort, "webpack/hot/dev-server")
}
config.plugins.push(new webpack.HotModuleReplacementPlugin());//热替换
...
/*fs.watch('./src/view/', function() { // webpack --progress --hide-modules --colors --profile --display-modules
